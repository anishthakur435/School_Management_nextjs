...
export default function ParentsTablePage() {
  ...
  const handleDelete = (deleteId) => {
    ...
    //
    if (Number(parents.length) === Number(1)) {
  ...
  //
  const finalFilteredParents = parents.filter((user) => {
  ...
  const parentsDataColoums = [
    {
      ...
      render: (value, row) => (
        <Typography
          className=""
          component={Link}
          href={`/dashboard/users/parents/${row.id}`}
        >
          {row.id}
        </Typography>
      ),
  ...
  return (
    <>
      <Paper elevation={3} className="rounded-2xl p-4 h-full flex flex-col">
        ...
        <ReusableTable
          columns={parentsDataColoums}
          data={finalFilteredParents}
        />
